...
void InitADC0()
{
    ...
    Master_Adc0_Config.CONFIG1 = ADLPC_NORMAL 			//No low power mode
								| ADC_CFG1_ADIV(ADIV_1)//ADIV_4) //divide input by 4
								| ADLSMP_SHORT 		//ADLSMP_LONG	//long sample time
								| ADC_CFG1_MODE(MODE_8)//single ended 8-bit conversion
								| ADC_CFG1_ADICLK(ADICLK_BUS);//clock choose
    
    Master_Adc0_Config.CONFIG2 = MUXSEL_ADCA // select the A side of the ADC channel.
								| ADACKEN_DISABLED //asynchronous clock disabled
								| ADHSC_HISPEED
...
void ADC0_IRQHandler()
{
	...
	switch(CurrentADC_State)
	{
		...
		case ADC_STATE_CAPTURE_BATTERY_LEVEL:
				...
				for(Junk = 0;Junk<TAOS_CLK;Junk++)
				{
				}
				
				TAOS_SI_LOW;

				//prekonfiguracja odczytow z kamery
				CurrentLineScanPixel = 0;
				CurrentADC_State = ADC_STATE_CAPTURE_LINE_SCAN;
				LineScanPos0WorkingBuffer->df_a=0;
				LineScanPos0WorkingBuffer->df_b=0;
				LineScanPos0WorkingBuffer->b=LINE_SCAN_WRONG_POS;
				ADC0_CFG2  |= ADC_CFG2_MUXSEL_MASK; //Select the B side of the mux
		...
		case ADC_STATE_CAPTURE_LINE_SCAN:
					...
					if(CurrentLineScanPixel<128)
					{
						...
						if(CurrentLineScanPixel==0)
						{
							LineScanDerivative0WorkingBuffer[CurrentLineScanPixel]=
							LineScanCentralDerivative(LineScanImage0WorkingBuffer[0],
													  LineScanImage0WorkingBuffer[1]);	
						}
						else if(CurrentLineScanPixel<127)
						{
							LineScanDerivative0WorkingBuffer[CurrentLineScanPixel] = 
									LineScanCentralDerivative(LineScanImage0WorkingBuffer[CurrentLineScanPixel-1],
															  LineScanImage0WorkingBuffer[CurrentLineScanPixel+1]);	
						}
						else /*CurrentLineScanPixel==127*/
						{
							LineScanDerivative0WorkingBuffer[CurrentLineScanPixel]=
								LineScanCentralDerivative(LineScanImage0WorkingBuffer[126],
														  LineScanImage0WorkingBuffer[127]);
						}
						
						//Znajdowanie ekstremum pochodnej
						if(LineScanDerivative0WorkingBuffer[CurrentLineScanPixel] < LineScanPos0WorkingBuffer->df_a)
						{
							LineScanPos0WorkingBuffer->a = CurrentLineScanPixel;
							LineScanPos0WorkingBuffer->df_a = LineScanDerivative0WorkingBuffer[CurrentLineScanPixel];		
						}
						else if(LineScanDerivative0WorkingBuffer[CurrentLineScanPixel] > LineScanPos0WorkingBuffer->df_b)
						{
							LineScanPos0WorkingBuffer->b = CurrentLineScanPixel;
							LineScanPos0WorkingBuffer->df_b = LineScanDerivative0WorkingBuffer[CurrentLineScanPixel];
						}
					...
					else
					{
						...
						if(LineScanWorkingBuffer == 0)
						{
							...
							LineScanDerivative0WorkingBuffer = &LineScanDerivative0Buffer[1][0];						
							LineScanDerivative0 = &LineScanDerivative0Buffer[0][0];
							
							LineScanPos0WorkingBuffer = &linePos[1];
							LineScanPos0 = &linePos[0];
						}
						else
						{
